...
def main(rank1):
    ...
    adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask = load_data_original(
        FLAGS.dataset)
    ...
    for epoch in range(FLAGS.epochs):
        ...
        for batch in iterate_minibatches_listinputs([normADJ_train, y_train],
            ...
            if rank1 is None:
                ...
                if rank1 > len(distr):
                    ...
                    q1 = np.random.choice(distr, rank1, replace=False,
                                          p=p0[distr] / sum(p0[distr]))  # top layer

                support1 = sparse_to_tuple(normADJ_batch[:,
                                                         q1].dot(sp.diags(1.0 / (p0[q1] * rank1))))
                if len(support1[1]) == 0:
                    continue
        ...
        # Validation
        cost, acc, _, duration = evaluate(val_features, valSupport, y_val, placeholders)
        ...
        print("Epoch:", '%04d' % (epoch + 1), "train_loss=", "{:.5f}".format(outs[1]), "train_acc=",
              ...
              "{:.5f}".format(acc), "time per batch=", "{:.5f}".format((time.time() - t1) / n))

        if epoch % 5 == 0:
            # Validation
            test_cost, test_acc, _, test_duration = evaluate(test_features, testSupport, y_test,
                                                             placeholders)
            print("training time by far=", "{:.5f}".format(time.time() - t),
                  "epoch = {}".format(epoch + 1), "cost=", "{:.5f}".format(test_cost), "accuracy=",
                  "{:.5f}".format(test_acc))
    ...
    test_cost, test_acc, test_y_pred, test_duration = evaluate(test_features, testSupport, y_test,
                                                               placeholders)
    print("rank1 = {}".format(rank1), "cost=", "{:.5f}".format(test_cost), "accuracy=",
          "{:.5f}".format(test_acc), "training time=", "{:.5f}".format(train_duration),
          "training time per epoch=", "{:.5f}".format(train_duration / (epoch + 1)), "test time=",
